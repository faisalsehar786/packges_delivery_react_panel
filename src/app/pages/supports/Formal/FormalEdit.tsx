...
import {ErrorMessage, Field, Form, Formik, FormikValues, useField} from 'formik'
...
export default function FormalEdit() {
  ...
  const memberSchema = Yup.object().shape({
    ...
    target_amount: Yup.string().matches(numberRegExp, 'Kun tall er tillatt i dette feltet. Bokstaver og andre symboler er ikke tillatt.'),
  ...
  let setDate = function (field: any, date: any) {
    if (field == 'startDate') {
  ...
  const MyTextArea = ({...props}: any) => {
    ...
    return (
      <>
        <textarea className='form-control form-control-lg ' {...field} {...props} />
  ...
  const submitStep = async (values: any, actions: FormikValues) => {
    let  reFactorValue=`'${values?.target_amount}'`;
    values.target_amount=Number(reFactorValue?.replace(/[~`!@#$%^&*()+={}\[\];:\'\"<>.,\/\\\?-_]/g, ''));
    const body = {
      ...values,
      start_date: startDate,
      due_date: endDate,
    }
    const {data} = await handlePatchRequest(`/goal/update_goal_admin/${id}`, body, {})(setLoading)
    patchForm(data?.data)
                ...
                <Form>
                  <div className=''>
                    <div>
                      ...
                      <div className=' fontsizechfslogtime d-flex align-items-center text-gray-400  me-5 mt-6 '>
                        Bruk skjemaet under til å opprette nytt formål. Angi formålet en tittel,
                        velg start og slutt dato og definer hvor mye støtte du trenger. <br /> Skriv
                        en god beskrivelse som støttespillere kan bruke til å forstå hva støtten går
                        til.
                      </div> 
                    </div>
                  ...
                  <div className='col-md-10 offset-md-2 d-flex justify-content-between'>
                    ...
                    <button type='submit' className='btn btn-sm btn-primary'>
                      <span className='indicator-label'>Oppdater formål</span>
                    </button>
                  </div>
                </Form>
              )}
