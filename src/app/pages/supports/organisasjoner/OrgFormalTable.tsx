...
const TableRow: React.FC<TableRowProps> = ({goal, hideOrg, logo}) => {
  ...
  return (
    ...
    >
      ...
      <td className='text-muted fw-semibold d-block'>
        {' '}
      ...
      <td>
        <span className='text-dark fw-bold d-block mb-1 fs-6'>
          {numberSpacing(Number(goal?.total_active_supporter_player ?? 0))}
        </span>
      </td>
      <td>
        <span className='text-dark fw-bold d-block mb-1 fs-6'>
...
const OrgFormalTable: React.FC<Props> = ({org_id, search, hide, logo}) => {
      ...
      <table className='table align-middle gs-0 gy-4 hoverTable'>
        <thead>
          <tr className='fw-bold text-muted bg-light'>
            ...
            <th className='pointer' style={{width: '15%'}} onClick={() => handleSort('start_date')}>
              OPPSTART
              {sortKey === 'start_date' && (
                <>
                  {sortOrder === 'ASC' ? (
            ...
            <th
              className='pointer '
              style={{width: '15%'}}
              onClick={() => handleSort('total_active_supporter_player')}
            >
              STØTTESPILLERE
              {sortKey === 'total_active_supporter_player' && (
                <>
                  {sortOrder === 'ASC' ? (
                    <FaSortUp className='ms-2' />
                  ) : (
              ...
              {sortKey !== 'total_active_supporter_player' && <FaSort className='ms-2' />}
            </th>
            <th
              className='pointer '
              style={{width: '15%'}}
...
export default React.memo(OrgFormalTable)
