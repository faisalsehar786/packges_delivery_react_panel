...
export const handlePostRequest =
  ...
  async (setLoading: any) => {
    ...
    try {
      ...
      toast.success(response.data.message || 'Sletting utført.', { autoClose: 20000 })
...
export const handleGetRequestWithoutRefresh = (url: string) => async (setLoading: any) => {
  setLoading(true)
  const fetchUrl = `${process.env.REACT_APP_BACKEND_API_URL}${url}`
  try {
    const response = await axios.get(fetchUrl, {
...
export const handleGetRequestWithRefreshTokenHeader = (url: string) => async (setLoading: any) => {
  setLoading(true)
  const fetchUrl = `${process.env.REACT_APP_BACKEND_API_URL}${url}`
  try {
    const response = await axios.get(fetchUrl, {
...
export const handleDeleteRequest =
  ...
  async (setLoading: any) => {
    ...
    try {
      ...
      toast.success(response.data.message || 'Sletting fullført.', { autoClose: 20000 })
      return response.data
    } catch (error: any) {
      if (error.response.status === 401) return handleRefreshToken()
      toast.error(error.response.data.message || 'Noe gikk galt. Vennligst prøv igjen.', {
