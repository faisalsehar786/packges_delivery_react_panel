...
export function SetPassword() {
  ...
  const getRequestDetails = async () => {
    ...
    if ((!success && message === 'Link utløpt') || (success && message === 'Link utløpt')) {
      toast.error(
        `
        Link utløpt
      `,
  ...
  const submitStep = async (values: any) => {
    ...
    const { success } = await handlePostRequestWithoutMessage(`/admin/change-password`, {
    ...
    if (success) {
      toast.success(
        `
      Bruker opprettelse fullført. Du vil nå bli sendt til innloggingssiden hvor du kan logge inn med ditt nye passord.')
      `,
        { autoClose: 3000 }
      )
      setTimeout(() => {
      ...
      {({ values, setFieldValue }) => (
        <Form>
          <div className=' mb-10'>
            ...
            <h1 className='authtext'>Opprett passord</h1>
            {/* end::Title */}

            {/* begin::Link */}
            <div className='text-gray-400'>
              Skriv inn og bekreft password for å fullføre konto opprettelse.
            </div>
            {/* end::Link */}
          </div>
          <div className='fv-row mb-4'>
            <label className={clsx('col-lg-4 col-form-label fw-bold fs-6')}>Nytt passord</label>
            <Field
