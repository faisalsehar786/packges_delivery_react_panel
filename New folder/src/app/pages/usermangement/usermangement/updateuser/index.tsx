...
const UpdateUserDetails: React.FC = () => {
  ...
  const formik = useFormik<any>({
    ...
    onSubmit: async (values, { setErrors }) => {
      ...
      const { data } = await handlePatchRequest(`/admin/${id}`, formData, {})(setLoading)
      if (data) {
        navigation('/user-mangement/user-overview')
      }
    },
  })
  ...
  useEffect(() => {
    if (!id) return
    const getUser = async () => {
      const { data } = await handleGetRequest(`/admin/get_by_id/${id}`)(setLoading)
      setData({
