  ...
  return (
    <tr key={goal?._id}>
      <td>
        <div className='d-flex align-items-center'>
          ...
          <div className='d-flex justify-content-start flex-column'>
            ...
            <span className='text-muted fw-semibold d-block'>{goal?.order?.order_no}</span>
          </div>
        </div>
      </td>
      <td>
      ...
      <td className='text-end'>
        ...
        >
          {goal?.order?.order_status === 'awaiting_for_payment' && 'awaiting for payment'}
  ...
  useEffect(() => {
    const getGoals = async () => {
      ...
      try {
        ...
        const { data } = await handleGetRequest(endpoint, { params })(setLoading)
      ...
      <table className='table align-middle gs-0 gy-4 hoverTable'>
        <thead>
          <tr className='fw-bold text-muted bg-light'>
            ...
            <th className='pointer text-end rounded-end pe-6' style={{ width: '20%' }}>
              STATUS
            </th>
          </tr>
        </thead>
...
export default React.memo(ActiveTenderTable)
